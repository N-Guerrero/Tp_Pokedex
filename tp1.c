...
int main(int argc, char const *argv[])
{
	...
	while (leer_linea_csv(data, 5, funciones, punteros) == 5) {
		...
		if (pokedex_agregar_pokemon(kanto, nuevo_pokemon) == false)
			printf("El pokemon %s se escapo!!\n",
			       nuevo_pokemon.nombre);
		else
			printf("Capturaste a %s!!!\n", nuevo_pokemon.nombre);

		free(nombre);
	}

	printf("mostrando Pokedex...\n\n");
	size_t pokemons_mostrados =
		pokedex_iterar_pokemones(kanto, (&mostrar_pokemons), NULL);
